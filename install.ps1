...
function Get-ConfigurationPreferences {
    ...
    for ($i = 0; $i -lt $editors.Count; $i++) {
        $marker = if ($editors[$i].Name -eq "Neovim") { " (recommended)" } else { "" }
        Write-Host "    [$($i + 1)] $($editors[$i].Name)$marker" -ForegroundColor White
    }
    ...
    Write-Host ""
    Write-Host "    [Y] Enable predictions (recommended)" -ForegroundColor White
